...
import com.mustafaiev.tair.dressin.R;

/**
 * Created with IntelliJ IDEA.
 * User: tair_mustafaiev
 * Date: 5/16/13
 * Time: 12:33 AM
 * To change this template use File | Settings | File Templates.
 */
public class PropositionCardsHelper {
    ...
    public PropositionCardsHelper(Context context, int swipeDirection, Callback callback, float densityScale, float pagingTouchSlop) {
        mCallback = callback;
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mCurrView != null) {
                    ...
                    View button = null;//mCurrView.findViewById(R.id.onoff);
    ...
    private int determineDuration(View animView, float newPos, float velocity) {
        ...
        if (velocity != 0) {
            duration = Math.min(duration, (int) (Math.abs(newPos - animView.getTranslationX()) * 1000f / Math.abs(velocity)));
        } else {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mCurrView != null) {
                    ...
                    // the swipe.
                    if (!mDragging && deltaY > MIN_VERT && (Math.abs(deltaX)) < MIN_LOCK && deltaY > (FACTOR * Math.abs(deltaX))) {
                        mCallback.onScroll();
                        return false;
                    }
            ...
            case MotionEvent.ACTION_CANCEL:
                if (mCurrView != null) {
                    ...
                    // width
                    boolean childSwipedFastEnough = (Math.abs(velocity) > escapeVelocity) && (Math.abs(velocity) > Math.abs(perpendicularVelocity)) && (velocity > 0) == (mCurrAnimView.getTranslationX() > 0) && translation > 0.05 * currAnimViewSize;
                    if (LOG_SWIPE_DISMISS_VELOCITY) {
                        Log.v(TAG, "Swipe/Dismiss: " + velocity + "/" + escapeVelocity + "/" + perpendicularVelocity + ", x: " + translation + "/" + currAnimViewSize);
                    }
